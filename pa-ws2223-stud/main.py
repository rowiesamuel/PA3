...
if __name__ == "__main__":
    ...
    data_file = 'pa-ws2223-stud/project/data/data.hdf5' #change to original file name
    features_of_interest = ['cyan_dp','cyan_q']

    test = pd.HDFStore(data_file, mode='r')
    df = test.get_storer('/run1/Kennlinie_ESP_c_540_deg16.5')
    print(df.attrs._v_attrnames)
    print(df.attrs.cyan_dp_einheit)
    print(df.attrs.cyan_dp_groesse)
    print(df.attrs.cyan_pumpe_drehzahl)
    print(df.attrs.cyan_pumpe_drehzahl_einheit)
    print(df.attrs.cyan_q_einheit)
    print(df.attrs.cyan_q_groesse)
    print(df.attrs.cyan_q_unsicherheitsintervall)
    ...
    test_df = functions.get_df(data_file, '/run1/Kennlinie_ESP_c_1260_deg16.5/')
